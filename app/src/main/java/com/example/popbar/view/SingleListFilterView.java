...
public class SingleListFilterView extends RelativeLayout implements ViewBaseAction {
	 ...
	 * 数据集合，构造时传入
	 */
	private List<GroupsResponse.Nodes> data = new ArrayList<>();// 显示字段
	/**
	 * 监听实现接口
	 */
	 ...
	 *            默认显示的文本
	 */
	public SingleListFilterView(Context context, List<GroupsResponse.Nodes> data, String show) {
		super(context);
		this.showText = show;
		this.data = data;
		init(context);
		...
		listViewAdapter.setOnItemClickListener(new TextAdapter.OnItemClickListener() {
			...
			public void onItemClick(View view, int position) {
				if (position < data.size()) {
					showText = data.get(position).getName();
