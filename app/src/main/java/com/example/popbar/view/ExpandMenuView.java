...
//			int maxHeight = (int) (displayHeight * 0.5);
			int maxHeight = dip2px(mContext,48)*6+7;//显示高度-5个item的高度
			...
			line.setBackgroundResource(R.color.text_black_54);
...
//			popRootLayout.setBackgroundColor(mContext.getResources().getColor(R.color.transparency));
	...
	private void showPopup(int position) {
		...
		if (textView!=null){
			popupWindow.showAsDropDown(textView, displayWidth, 1);
		}else{
...
//		backgroundAlpha(0.4f);
	...
	private void backgroundAlpha(float f) {
		...
		lp.height -= dip2px(mContext,50)+popupWindow.getHeight();
		lp.alpha = f;
		((Activity) mContext).getWindow().setAttributes(lp);
//		((Activity) mContext).getWindow().get
	}
